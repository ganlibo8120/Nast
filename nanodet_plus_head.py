...
class NanoDetPlusHead(nn.Module):
    ...
    def loss(self, preds, gt_meta, aux_preds=None):
        ...
        # todo 辅助损失其实可以不加extraloss

        loss, loss_states = self._get_loss_from_assign(
            cls_preds, reg_preds, decoded_bboxes, batch_assign_res,1
        )
    ...
    def _get_loss_from_assign(self, cls_preds, reg_preds, decoded_bboxes, assign,f):
        ...
        if f:
            ...
            IOU[pos_inds] = Iou
            loss_extra = extra_loss(cls_preds, (labels, label_scores,IOU), pos_inds)
...
import random
def extra_loss(pred, target,labelBool):
    pred = pred.sigmoid()
    ...
    low = random.randint(num//10,num-2)
    ...
    for i in range(low,high):
        if  label0[i] not in allC:
            ...
            loss = (iouc-0.95)*torch.log( torch.min(1+predc-scorc,torch.tensor(1,device='cuda')))
            # 0.95
            #loss = loss[loss>=0]
            if iouc.size(0) > 0:
    ...
    return 0.25*Loss/max(len(allC),1) #torch.tensor(0.0,device='cuda')  if Loss[bol].size(0)==0 else  0.5* torch.mean(Loss[bol])
